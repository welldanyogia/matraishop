...
export default function TopCategories({ products }) {
    ...
    console.log(products.length - productsPerPage - currentIndex);
    ...
    const handleNext = () => {
        if (
            currentIndex <=
            products.length - productsPerPage - currentIndex * 2
        ) {
            setCurrentIndex((prevIndex) => prevIndex + 1);
        }
        console.log(currentIndex);
        ...
        <>
            ...
            >
                <div className={'relative w-3/5 overflow-hidden max-sm:w-full'}>
                    ...
                    <div
                        className="relative flex gap-4 p-4 transition-transform duration-300 ease-in-out"
                        style={{
                    ...
                    >
                        {products.map((product, index) => (
                            <div
                                key={product.id}
                                className="shadow-rounded group flex flex-col rounded-xl border bg-white p-2 shadow-xl shadow-[#F1F1E9] transition-transform duration-300 hover:scale-105 hover:shadow-md sm:w-[50%] md:w-[33.33%] lg:w-[25%] xl:w-[20%]"
                            >
                                <img
